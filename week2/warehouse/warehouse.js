...
const specialPackaging = document.getElementById("specialPackaging");
const hazardousMaterials = document.getElementById("hazardousMaterials");
const smallItemsOlny = document.getElementById("smallItemsOlny");
const forkiftNeeded = document.getElementById("forkiftNeeded");
const totalItems = document.getElementById("totalItems");
let sum = 0;

parts.forEach(function (part) {
  let items = part.qty + " " + " (" + part.partNbr + ") " + " - " + part.partDescr + "<br>";
  detailList.innerHTML += items;

  if (part.aisle === "B3") {
    let item = " item: " + part.partNbr + " / Qty: " + part.qty;
    specialPackaging.innerHTML += item;  
  } 

  if (part.aisle === "J4") {
    let item = " Get gloves";
    hazardousMaterials.innerHTML += item;
  } 

  if (part.aisle === "H1") {
    smallItemsOlny.remove();
  }
